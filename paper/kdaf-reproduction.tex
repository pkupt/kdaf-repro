...
\subsection{Extension Experiments: Dense Retrieval and Cross-Model Generation}\label{sec:ext}
...
\textbf{Traceability is retriever-determined by construction.} In the author's framework, citations are recorded at retrieval time: the runner attaches the retrieved evidence identifiers to each answer (\texttt{"citations": res.citations} in \texttt{run\_eval.py}), and the generator outputs only the answer string. Traceability F1 is therefore scored purely against the retrieval-side citation list, so any generator replaying the same retrieval cache necessarily yields identical traceability. The cross-model runs confirm this to the digit: per-question traceability is identical on 145/145 questions for both \texttt{dense\_bge} (0.4317) and the KDAF graph retriever (0.5147), while correctness moves (0.1241 $\to$ 0.1103 and 0.1241 $\to$ 0.1448, respectively). The empirically meaningful cross-model variation is thus confined to correctness; reported traceability differences across systems reflect the retrieval pipeline, not the generator choice.
...
The reproduction core (data normalization, graph construction, all author-supplied retrievers, the CARP pipeline, and scoring) runs the author's artifact (Zenodo 10.5281/zenodo.22022068, v1.0.0) unmodified, except for the one-line \texttt{think: False} adaptation documented in Section~\ref{sec:adapt}; Windows-specific shims live outside the artifact and only stub missing Unix APIs. The extension experiments of Section~\ref{sec:ext} additionally add: a new dense-retrieval module (\texttt{dense\_retriever.py}), its registration in \texttt{run\_eval.py} (one added import line plus a single registry entry in \texttt{make\_retrievers}), and new variant configuration files---none of which alter the reproduction path or its recorded numbers. Everything needed to re-run our record is available at \url{https://github.com/pkupt/kdaf-repro}: (a) the environment shim, (b) the metrics-comparison script used to verify exact behavioral equivalence, (c) per-question generation event logs for all six generation runs (benchmark question text redacted per FinanceBench redistribution terms; answers, citations, timings, and scores intact), (d) the dense-retrieval module, its wiring instructions, and the variant configurations used in our runs, and (e) this manuscript's source.
...
\bibitem{edge2024}
Edge, D., Trinh, H., Cheng, N., Bradley, J., Chao, A., Mody, A., Truitt, S., Metropolitansky, D., Ness, R. O., \& Larson, J. (2024). From Local to Global: A Graph RAG Approach to Query-Focused Summarization. \emph{arXiv:2404.16130}.

\bibitem{han2025}
Han, H., Wang, Y., Shomer, H., Guo, K., Ding, J., Lei, Y., Halappanavar, M., Rossi, R. A., Mukherjee, S., Tang, X., He, Q., Hua, Z., Long, B., Zhao, T., Shah, N., Javari, A., Xia, Y., \& Tang, J. (2025). Retrieval-Augmented Generation with Graphs (GraphRAG). \emph{arXiv:2501.00309}.

\bibitem{gao2023}
Gao, Y., Xiong, Y., Gao, X., Jia, K., Pan, J., Bi, Y., Dai, Y., Sun, J., Wang, M., \& Wang, H. (2023). Retrieval-Augmented Generation for Large Language Models: A Survey. \emph{arXiv:2312.10997}.

\bibitem{sr117}
Board of Governors of the Federal Reserve System \& Office of the Comptroller of the Currency. (2011). Supervisory Guidance on Model Risk Management (SR Letter 11-7 / OCC Bulletin 2011-12).

\bibitem{nist_ai_rmf}
Tabassi, E. (2023). Artificial Intelligence Risk Management Framework (AI RMF 1.0) (NIST AI 100-1). National Institute of Standards and Technology. \url{https://doi.org/10.6028/NIST.AI.100-1}

\bibitem{lee2004}
Lee, J. D., \& See, K. A. (2004). Trust in Automation: Designing for Appropriate Reliance. \emph{Human Factors}, 46(1), 50--80. \url{https://doi.org/10.1518/hfes.46.1.50_30392}
